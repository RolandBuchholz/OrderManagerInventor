...
namespace OrderInvAddin.Addin
{
    public static class AddinUtilities
    {
                                                                       ...
                                                                       string IconFolder = "")
        {
            ...
            if (!(testDef == null))
            {
                MessageBox.Show("Error when loading the add-in \"OrderManager\". A command already exists with the same internal name. Each add-in must have a unique internal name. Change the internal name in the call to CreateButtonDefinition.", "OrderManager Inventor Add-In");
                return null;
            }
